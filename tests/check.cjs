...
(async () => {
  ...
  await page.waitForFunction(() => document.querySelector('#overview-grid').children.length === 44);
  assert.equal(await page.locator('.slide').count(), 44);
  ...
  await go(20); await page.locator('#xmss-sign').click(); await page.locator('#xmss-backup').click(); await page.locator('#xmss-sign').click(); assert.match(await page.locator('#xmss-result').innerText(), /reutilizado/); await page.locator('#xmss-reset').click(); for (let i = 0; i < 8; i++) await page.locator('#xmss-sign').click(); assert(await page.locator('#xmss-sign').isDisabled());
  await go(21); await page.locator('[data-size-mode=total]').click(); assert.match(await page.locator('#size-chart').innerText(), /3\.732 B/); await page.locator('[data-size-mode=signature]').click(); assert.match(await page.locator('#size-chart').innerText(), /2\.420 B/);
  await go(34); await page.locator('[data-exposure=public]').click(); assert.match(await page.locator('#exposure-window').innerText(), /prolongada/);
  await go(35); assert.equal(await page.locator('#volume-pq').innerText(), '2,42 GB'); await page.locator('#signature-count').fill('10'); assert.equal(await page.locator('#volume-pq').innerText(), '24,2 GB'); await page.locator('#scheme-select').selectOption('3'); assert.equal(await page.locator('#volume-pq').innerText(), '78,56 GB');
  await go(37); await page.locator('[data-decision=protect]').click(); assert.match(await page.locator('#decision-result').innerText(), /legítimos/);
  await go(42); for (const answer of [3, 3, 4]) { await page.locator('.quiz-option').nth(answer).click(); await page.locator('#quiz-next').click(); } assert.match(await page.locator('#quiz-feedback').innerText(), /3 de 3/); await page.locator('#quiz-restart').click(); assert.match(await page.locator('#quiz-number').innerText(), /01/);
  ...
  for (const n of [4, 10, 16, 20, 21, 22, 34, 35, 37, 42]) {
    await go(n);
  ...
  await go(8);
  await page.locator('body').click({ position: { x: 10, y: 10 } }); await page.keyboard.press('ArrowRight'); assert.equal(await page.locator('#slide-current').innerText(), '09'); await page.keyboard.press('Home'); assert.equal(await page.locator('#slide-current').innerText(), '01'); await page.keyboard.press('End'); assert.equal(await page.locator('#slide-current').innerText(), '44'); assert(await page.locator('#next').isDisabled());
  ...
  console.log(JSON.stringify({ slides: 44, errors, desktopOverflow: layout, interactionChecks: 'passed', mobileChecks: 'passed' }, null, 2));
  assert.deepEqual(errors, []);
  assert.deepEqual(layout, [], 'Desktop slides must fit without scroll');
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.emulateMedia({ media: 'print' });
  assert.equal(await page.locator('.slide:visible').count(), 44);
