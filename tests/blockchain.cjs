...
(async () => {
  ...
  await page.locator('#poh-tx').fill('Eva → Fabio · 3 SOL'); await page.locator('#poh-add').click();
  await page.waitForFunction(() => [...document.querySelectorAll('#poh-stream .has-tx')].some(entry => +entry.dataset.txCount >= 5));
  assert((await page.locator('#poh-stream > article').count()) <= 3);
